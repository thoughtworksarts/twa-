...
function getPublishingConfig() {
  ...
  const getAllReadableRemindersAsHtml = () => {
    ...
    for(const channel in publicationResponses) {
      html += '<tr><td><pre>' + channel.replace(': ', '<br>') + '</pre></td><td><pre>' + getReadableReminders(publicationResponses[channel].reminderTexts, '<br>') + '</pre></td><tr>';
    }
    return html + '</table>';
  };
