...
function getPublishingConfig() {
  ...
  const promptResponses = {
    "TinyLetter: Newsletter": (row) => {
      const feature = state.builder.findFeature('insert twa.io newsletter for andy');
      feature.config.values = [row['B'], 'TWA.io: Newsletter', '(0) Not Started', '', 'Generated by TinyLetter publication'];
      feature.execute();
    }
  }
    ...
    features: {
      ...
      alertSheetOnEdit: {
        ...
        respondToPrompt(button, row) {
          const channel = row['C'];
          promptResponses[channel](row);
        }
      },
      createSheetItem: {
        id: 'insert twa.io newsletter for andy',
        insertionSectionIndex: sectionIndexGetter('Andy')
      }
    },
    sidebar: {
      guidance: {
