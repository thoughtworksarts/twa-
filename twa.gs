...
function getPublishingConfig() {
  ...
  channelReminders['TinyLetter: Newsletter'] = ['TWA.io clone'];
  channelReminders['TWA.io: Blog'] = ['Medium clone', 'Social shares', 'Contact associates'];
  channelReminders['TWA.io: Project'] = ['Social shares', 'Contact associates'];

  const readableReminders = (reminders, newLine='\n', bullet='• ') => { return bullet + reminders.join(newLine + bullet); };
  let allReadableReminders = '<table>';
  for(const channel in channelReminders) {
    allReadableReminders += '<tr><td><pre>' + channel.replace(': ', '<br>') + '</pre></td><td><pre>' + readableReminders(channelReminders[channel], '<br>') + '</pre></td><tr>';
  }
  allReadableReminders += '</table>';

  const sectionIndexGetter = (sectionName) => {
    const sectionIndices = { Paige: 1, Andy: 2 }
    return sectionIndices[sectionName];
  };

//Plex channelReminders into this

  const getGeneratedBy = (channel) => {
    return 'Generated by publication of ' + channel;
  }

  const getStatus = (sheetName) => {
    if(sheetName === 'Publishing') return '(0) Not Started';
    if(sheetName === 'Current:Paige') return '(1) Now';
    throw `Don't recognize sheet '` + sheetName + `'`;
  }

  const executeFeature = (id, values, section=false) => {
    const feature = state.builder.findFeature(id) || false;
    logString(feature.config.id);
    feature.config.values = values;
    if(section) feature.config.insertionSectionIndex = sectionIndexGetter(section);
    feature.execute();
  }
  ...
  const publicationResponses = {
    "TinyLetter: Newsletter": {
      executeFeatures: (channel, triggerText) => {
        executeFeature(get.featureId.createInPublishing,   [get.instruction.createTwaNewsletter(triggerText), get.channel.twaNewsletter, getStatus(get.sheet.publishing), '', getGeneratedBy(channel)], 'Andy');
      }
    },
    "TWA.io: Blog": {
      executeFeatures: (channel, triggerText) => {
        executeFeature(get.featureId.createInPublishing,   [get.instruction.createMedium(triggerText), get.channel.mediumBlog, getStatus(get.sheet.publishing), '', getGeneratedBy(channel)], 'Paige');
        executeFeature(get.featureId.createInPublishing,   [get.instruction.createSocial(triggerText), get.channel.social,     getStatus(get.sheet.publishing), '', getGeneratedBy(channel)], 'Paige');
        executeFeature(get.featureId.createInCurrentPaige, [get.instruction.contactAssociates(channel, triggerText), getStatus(get.sheet.currentPaige), get.instruction.contactHow, get.instruction.contactWhenDone, '', '']);
      }
    },
    "TWA.io: Project": {
      executeFeatures: (channel, triggerText) => {
  ...
  return {
    ...
    features: {
      ...
      alertSheetOnEdit: {
        ...
        getMessage: (row) => {
          const channel = row['C'];
          const reminders = channelReminders[channel];
          if(isProperty(reminders)) return { title: 'Publication Triggers', text: 'You just published a ' + channel + ', which should trigger the following:\n\n' + readableReminders(reminders) + '\n\nWould you like to create these entries now?' };
          return false;
        },
      ...
      create: {
        ...
        features: {
          createSheetItem: {
            events: [Event.onSidebarSubmit],
            priority: 'HIGH_PRIORITY',
            getInsertionSectionIndex: sectionIndexGetter,
            getValues: () => {
